...
/**
		...
		*/
		pair<CDouble, list<CString *> *> MakespanCalculator::calculateWorstCaseMakespan(MaxPlusAutomaton *mpa)
		{
			// make sure the automaton has no cycles
			if (mpa->hasDirectedCycle())
				throw CException("calculateWorstCaseMakespan: automaton has directed cycle, can not compute makespan");

			MPASetOfStates *F = mpa->getFinalStates();
			if (F->size() < 1)
			{
				throw CException("calculateWorstCaseMakespan: automaton has no final state, can not compute makespan");
			}
			...
			pair<CDouble, list<CString *> *> makespan = bellmanFordMax(mpa, dummyState, false);
		...
		*/
		pair<CDouble, list<CString *> *> MakespanCalculator::bellmanFordMin(MaxPlusAutomaton *mpa, MPAState *srcId, bool checkForNegativeCycle = false)
		{
			map<MPAState *, pair<CDouble, list<CString *> *>> result = bellmanFord(mpa, srcId, checkForNegativeCycle);

			MPASetOfStates *F = mpa->getFinalStates();
			CDouble minDist = 0;
			MPAState *minState = 0;
			for (map<MPAState *, pair<CDouble, list<CString *> *>>::iterator i = result.begin(); i != result.end(); i++)
			{
		...
		* returns only makespan of accepted words
		*/
		pair<CDouble, list<CString *> *> MakespanCalculator::bellmanFordMax(MaxPlusAutomaton *mpa, MPAState *srcId, bool checkForNegativeCycle = false)
		{
		...
		void bellmanFordThread(MPASetOfEdges *edges, map<MPAState *, pair<CDouble, list<CString *> *>> &dis)
		{
			while (true)
			{
				for (MPASetOfEdges::CIter j = edges->begin(); j != edges->end(); j++)
				{
					...
					if (dis[sourceId].first != INFINITY)
					{
						...
						{
							list<CString *> *scList = new list<CString *>(*dis[sourceId].second);
		...
		void bellmanFordStaticSequential(CDouble* vDst, struct EdgeData* edges, size_t nrVertices, size_t nrEdges)
		{
		...
		void bellmanFordFindCriticalEdges(CDouble* vDst, struct EdgeData* edges, size_t* critical, size_t nrEdges)
		{
			for (size_t i = 0; i < nrEdges; i++) {
				EdgeData* e = &(edges[i]);
				size_t sourceId = e->sourceId;
				...
				if (sd + weight <= dd + MP_EPSILON) {
					if (sourceId != destId) { //hack: remove if loop on first state labelled DummyState is fixed
						critical[destId] = i;
					}
				}
			}
		...
		*/
		map<MPAState *, pair<CDouble, list<CString *> *>> MakespanCalculator::bellmanFord(MaxPlusAutomaton *mpa, MPAState *srcId, bool checkForNegativeCycle = false)
		{
			...
			map<MPAState*, pair<CDouble, list<CString*>*>> dis;
