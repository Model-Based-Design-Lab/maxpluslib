...
namespace MaxPlus
{
	...
	*/
	string loadEntity(ifstream& stream, string line)
	{
		string out = line;
	...
	MaxPlusAutomaton* SMPLS::convertToMaxPlusAutomaton()
	{
		...
		for (q = Q->begin(); q != Q->end(); q++)
		{
			...
			for (unsigned int k = 0; k < nrTokens; k++)
			{
				...
				mpa->addState(s);
				//cout << "DEBUG adding state to mpa id: " << (CString)(s->getLabel().id) << ", tkn: " <<  (s->getLabel().tokenNr)<< endl;
				if (isInitial)
			...
			for (e = t->begin(); e != t->end(); e++)
			{
				...
				for (size_t row = 0; row < r; row++)
				{
					for (size_t col = 0; col < c; col++)
					{
						...
						{
							MPAState* src = mpa->getStateLabeled(make_mpastatelabel(q1Id, (unsigned int)col));
							MPAState* dst = mpa->getStateLabeled(make_mpastatelabel(q2Id, (unsigned int)row));
	...
	*/
	...
	MaxPlusAutomaton* SMPLSwithEvents::convertToMaxPlusAutomaton()
	{
		...
		multiset<Event>* eventList = new multiset<Event>();
	...
	/**
		 ...
		 */
	void SMPLSwithEvents::prepareMatrices(IOAState* s, multiset<Event>* eventList, IOASetOfEdges* visitedEdges)
	{
		...
		for (i = adj->begin(); i != adj->end(); ++i)
		{
			...
			elsFSM->addEdge(elsFSM->findState(s->getLabel()), scenarioName, elsFSM->findState(((IOAState*)e->getDestination())->getLabel()));
			...
			if (eList->size() > 0)
			{
				...
				for (eventIter = eTempList->begin(); eventIter != eTempList->end(); eventIter++)
				{
					...
					else // or are we conveying this event to the next state vector? I_c(q) (refer to paper)
					{
						...
						sMatrix->put(sMatrix->getRows() - 1, sMatrix->getCols() - 1, 0); // and 0 in the intersection of that row and col
					}
					...
					{
						sMatrix->addRows(1);
						Matrix* eventRow = *dissm->eventRows->begin(); // for now we just assume max one event emitted per mode
	...
	/**
		 ...
		 */
	void SMPLSwithEvents::isConsistentUtil(IOAState* s, list<Event>* eventList, IOASetOfStates* finalStates, CString* errMsg, map<IOAState*, list<Event>*>* visited)
	{
			...
			for (i = adj->begin(); i != adj->end(); ++i)
			{
				...
				if (input != "")
				{
					...
					if (!eventFound)
					{
						...
						*errMsg = "on edge: " + to_string(s->getLabel()) + " - " + string(input + "," + output + ", Processing event outcome : " + input + " where the event is not emitted yet.\r\n");
						return;
					}
				}
				// we store the events emitted by modes as we move along the automaton
				if (output != "")
				{
					...
					for (sigmaIterator = sigma->begin(); sigmaIterator != sigma->end(); sigmaIterator++)
					{
						if (((pair<Mode, Event>) * sigmaIterator).first == output) // this output action emits an event
						{
	...
	void SMPLS::loadAutomatonFromIOAFile(CString fileName)
	{
		...
		while (getline(input_stream, line))
		{
			...
			if (i1 != std::string::npos && i2 != std::string::npos)
			{
				if (i1 < i2) // if we have a valid line as in "state i f --edge--> state2 f"
				{
					...
					string edgeDescription = line.substr(i1, i2 - i1);
		...
		cout << endl
			<< "automaton states size: " << elsFSM->getStates()->size() << endl;
	...
	void SMPLSwithEvents::loadIOAutomatonFromDispatchingFile(CString fileName)
	{
		...
		cout << endl
			<< "IO Automaton states size: " << ioa->getStates()->size() << endl;
			...
			if (i1 != std::string::npos && i2 != std::string::npos)
			{
				if (i1 < i2) // if we have a valid line as in "state i f --edge--> state2 f"
				{
					...
					if (tempIndex2 != std::string::npos)
						...
						throw CException("Can not split edge into input and output action. Edge description is invalid. at line: " + lineNumber);
