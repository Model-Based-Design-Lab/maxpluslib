...
void SMPLSwithEvents::prepareMatrices(const IOAState &s,
    ...
    for (const auto *i : adj) {
        ...
        this->elsFSM.addEdge(*this->elsFSM.getStateLabeled(s.getLabel()),
                             modeName,
                             *this->elsFSM.getStateLabeled(
                                     (dynamic_cast<IOAStateRef>(e->getDestination()))->getLabel()));
        ...
        if (!eList.empty()) {
            ...
            for (eventIter = eTempList.begin(); eventIter != eTempList.end(); eventIter++) {
                ...
                if (eventIndexCounter == processingEventIndex) // we add B_c and B_e
                {
                    ...
                    {
                        sMatrix->put(
                                y, cols - 1, getMaxOfRowUntilCol(*sMatrix, y, numberOfResources));
                    }

                    eList.erase(
                            eList.lower_bound(*eventIter)); // event is processed, remove from eList
                       ...
                       // paper)
                {
                    ...
                    sMatrix->put(sMatrix->getRows() - 1,
                                 sMatrix->getCols() - 1,
                                 MPTime(0)); // and 0 in the intersection of that row and col
                }
                // if we need to add the event row at the bottom
                if ((eventIndexCounter == emittingEventIndex)
                    || (eventIndexCounter + 1 == static_cast<int>(eTempList.size())
                        && emittingEventIndex
                                   == eventIndexCounter
                                                    ...
                                                    // need to add the event at the bottom
                {
                    ...
                    {
                        if (getMaxOfColUntilRow(*sMatrix, x, numberOfResources)
                            > MP_MINUS_INFINITY) {
                            // we must only add B_e under cols that correspond to B_c
                            sMatrix->put(sMatrix->getRows() - 1,
                                         x,
                                         getMaxOfRowUntilCol(*eventRow, 0, numberOfResources));
                        }
                    }
                }
        ...
        biggestMatrixSize =
                std::max(biggestMatrixSize, std::max(sMatrix->getCols(), sMatrix->getRows()));
...
void SMPLSwithEvents::isConsistentUtil(const IOAState &s,
                             ...
                             // inconsistency before leaving the state
    {
        if (!compareEventLists(it->second, eventList)) {
            errMsg = MPString("Different paths leading to different events at state "
                              + std::to_string(s.getLabel()));
        }
        ...
        for (const auto *i : adj) {
            ...
            if (!input.empty()) {
                ...
                if (!eventFound) {
                    ...
                    errMsg = MPString("on edge: " + std::to_string(s.getLabel()) + " - "
                                      + std::string(input + "," + output
                                                    + ", Processing event outcome : " + input
                                                    + " where the event is not emitted yet.\r\n"));
                    return;
                }
            }
            // we store the events emitted by modes as we move along the automaton
            if (!output.empty()) {
                ...
                for (const auto &sigmaIterator : this->sigma) {
                    if ((ModeEventPair(sigmaIterator)).first
